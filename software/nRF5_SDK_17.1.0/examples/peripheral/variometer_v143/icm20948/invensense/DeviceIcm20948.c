...
static int idd_driver_2_sensortype(enum inv_icm20948_sensor sensor) {
  ...
  case INV_ICM20948_SENSOR_ROTATION_VECTOR:
    return INV_SENSOR_TYPE_ROTATION_VECTOR;
  case INV_ICM20948_SENSOR_GEOMAGNETIC_ROTATION_VECTOR:
    return INV_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR;
  case INV_ICM20948_SENSOR_GEOMAGNETIC_FIELD:
    return INV_SENSOR_TYPE_MAGNETOMETER;
  case INV_ICM20948_SENSOR_WAKEUP_SIGNIFICANT_MOTION:
    return INV_SENSOR_TYPE_SMD;
  case INV_ICM20948_SENSOR_FLIP_PICKUP:
    return INV_SENSOR_TYPE_PICK_UP_GESTURE;
  case INV_ICM20948_SENSOR_WAKEUP_TILT_DETECTOR:
    return INV_SENSOR_TYPE_TILT_DETECTOR;
  case INV_ICM20948_SENSOR_GRAVITY:
    return INV_SENSOR_TYPE_GRAVITY;
  case INV_ICM20948_SENSOR_LINEAR_ACCELERATION:
    return INV_SENSOR_TYPE_LINEAR_ACCELERATION;
  case INV_ICM20948_SENSOR_ORIENTATION:
    return INV_SENSOR_TYPE_ORIENTATION;
  case INV_ICM20948_SENSOR_B2S:
    return INV_SENSOR_TYPE_B2S;
  default:
    return INV_SENSOR_TYPE_MAX;
  }
}
 ...
 */
static int host_serif_write_reg_legacy(void *context, uint8_t reg, uint8_t *data, uint32_t len) {
  inv_host_serif_t *serif = (inv_host_serif_t *)context;
...
#if (IMU_CHIP == HW_ICM20948)
  /*force the usage of akm9916 for 20948*/
  inv_device_icm20948_init_aux_compass(self, INV_ICM20948_COMPASS_ID_AK09916, 0x0C);
  //NRF_LOG_INFO("Init auxiliary compass AK09916");
  nrf_delay_ms(10);

#endif
}

int inv_device_icm20948_poll(void *context) { // NRF_LOG_DEBUG("inv_device_icm20948_poll");
...
int inv_device_icm20948_setup(void *context) {
  ...
  if ((rc = inv_device_icm20948_whoami(self, &whoami)) != 0) {
    ...
    NRF_LOG_ERROR("Error %d when reading WHOAMI value", rc);

    return rc;
  }

  /*if (whoami != 0x0EA) { // ICM@0948
    NRF_LOG_ERROR("Unexpected WHOAMI value 0x%x. Try turn off and on...", whoami);
    return INV_ERROR;
  } else {
    NRF_LOG_INFO("ICM WHOAMI value: 0X%02X ", whoami);
    // sprintf(ESBstring, "MPU who I am  (should be 0xEA): 0X%02X ,%d", whoami,  "Init error : ", rc);
    //  ESBsend();
  }*/
...
int inv_device_icm20948_self_test(void *context, int sensor) {
  ...
  if (self->icm20948_states.selftest_done) {
    NRF_LOG_INFO("Self-test already ran once!");
...
// int inv_device_icm20948_get_sensor_bias(void * context, int sensor, float bias[3])
...
static void data_handler(void *context, enum inv_icm20948_sensor sensor, uint64_t timestamp,
    const void *data, const void *arg) { // NRF_LOG_DEBUG("data_handler");
  inv_device_icm20948_t *self = (inv_device_icm20948_t *)context;
  inv_sensor_event_t event;
...
static inv_bool_t build_sensor_event_data(inv_device_icm20948_t *self,
    ...
    inv_sensor_event_t *event) { // NRF_LOG_DEBUG("build_sensor_event_data");
  ...
  case INV_SENSOR_TYPE_UNCAL_MAGNETOMETER:
    ...
    /* sprintf(ESBstring, "%.4f, %.4f, %.4f",raw_bias_data[0], raw_bias_data[1], raw_bias_data[2]);  ////*********
      NRF_LOG_RAW_INFO("UNCAL_MAGNETO data: %s\n", ESBstring);  //**********

      sprintf(ESBstring, "%.4f, %.4f, %.4f",event->data.mag.vect[0], event->data.mag.vect[1], event->data.mag.vect[2]);  ////*********
      NRF_LOG_RAW_INFO("UNCAL_MAGNETO vect: %s\n", ESBstring);
      sprintf(ESBstring, " %.4f, %.4f, %.4f",event->data.mag.bias[0], event->data.mag.bias[1], event->data.mag.bias[2]);  ////*********
      NRF_LOG_RAW_INFO( "Mag_biais : %s\n", ESBstring); */
...
static inv_bool_t build_sensor_event(inv_device_icm20948_t *self,
    int sensorid, uint64_t timestamp, const void *data, const void *arg,
    inv_sensor_event_t *event) { // NRF_LOG_DEBUG(" build_sensor_event ");
  assert(event);
