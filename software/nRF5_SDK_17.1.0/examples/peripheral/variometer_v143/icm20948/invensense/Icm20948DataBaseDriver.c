	...
	const uint8_t *dmp3_image, uint32_t dmp3_image_size)
{
	...
	// Enable Interrupts.
	data = 0x2;
	result |= inv_icm20948_write_mems_reg(s, REG_INT_ENABLE, 1, &data); //bank 0/ 0x10 Enable DMP_INT1_EN 
	data = 0x1; //data = 0x1; 
	result |= inv_icm20948_write_mems_reg(s, REG_INT_ENABLE_2, 1, &data); // bank 0/ 0x12  FIFO Overflow Interrupt enable ??
...
static unsigned char inv_is_gyro_enabled(struct inv_icm20948 * s)
{
	if ((s->inv_androidSensorsOn_mask[0] & INV_NEEDS_GYRO_MASK) || 
		(s->inv_androidSensorsOn_mask[1] & INV_NEEDS_GYRO_MASK1)){
		return 1;
	}
	return 0;
}
